...
export function Sidebar({ open = false, onClose }: { open?: boolean; onClose?: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex h-full w-64 shrink-0 -translate-x-full flex-col border-r border-surface-border bg-surface-base transition-transform duration-200 lg:static lg:translate-x-0 ${
          open ? "translate-x-0" : ""
        }`}
      >
        <div className="flex items-center justify-between border-b border-surface-border p-5">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1.5 text-muted-text transition-colors hover:bg-surface-card-hover hover:text-foreground lg:hidden"
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <nav className="mt-2 space-y-1">
            <NavItem href="/" icon={<PlusIcon />} label="New run" onNavigate={onClose} />
            <NavItem href="/runs" icon={<ListIcon />} label="My runs" onNavigate={onClose} />
            {isAdmin && <NavItem href="/access" icon={<KeyIcon />} label="Access" onNavigate={onClose} />}
          </nav>
        </div>
